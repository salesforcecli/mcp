...
export async function detectConflict({
  ...
  if (!workItem) {
    return {
      content: [{
        type: "text",
        text: "Error: Please provide a workItem to check for conflicts. Use the sf-devops-list-work-items tool to fetch work items first."
      }]
    };
  }
      ...
      content: [{
        ...
        1. For work item branch (${workItemBranch}):
          ...
          - If there are conflicts:
              ...
              - After these steps, call the sf-devops-resolve-conflict tool to resolve the conflicts.
        - IMPORTANT: Do not provide resolution instructions, call the sf-devops-resolve-conflict tool to resolve the conflicts.

Goal:
explain conflict in ${workItemBranch} with ${targetBranch} in simple langugae and call sf-devops-resolve-conflict tool to resolve the conflicts.
