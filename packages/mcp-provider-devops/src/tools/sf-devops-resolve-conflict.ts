...
const DESCRIPTION: string = `Resolves merge conflicts for a selected work item by name.
...
/**
 * MCP tool for resolving merge conflicts in work items.
 */
export class SfDevopsResolveConflictMcpTool extends McpTool<InputArgsShape, OutputArgsShape> {
    public static readonly NAME: string = 'sf-devops-resolve-conflict';
    ...
    public getName(): string {
        return SfDevopsResolveConflictMcpTool.NAME;
    }

    public getConfig(): McpToolConfig<InputArgsShape, OutputArgsShape> {
        return {
            title: "Resolve DevOps Merge Conflicts",
    ...
    public async exec(input: { username: string; workItemName: string; localPath?: string }): Promise<CallToolResult> {
        try {
            // First fetch the work item by name
            ...
            const response = {
                ...
                message: resultText,
                nextSteps: resolved ? "Run 'sf-devops-detect-conflict' to verify, then promote or create PR" : "Review conflict resolution guidance and try again"
            };
            ...
            const response = {
                resolved: false,
                message: `Error resolving conflicts: ${errorMessage}`,
