...
/**
 * Fetches all DevOps projects from the specified org.
 */
...
/**
 * Extracts repository information from a work item record.
 */
...
/**
 * Ensures project pipeline stages are cached and available.
 */
...
/**
 * Maps raw Salesforce work item data to structured WorkItem format.
 */
 ...
 */
export async function fetchWorkItems(username: string, projectId: string): Promise<WorkItem[]> {
    try {
        const connection = await getConnection(username);
        const query = `
            SELECT
...
/**
 * Fetches a specific work item by name.
 */
export async function fetchWorkItemByName(username: string, workItemName: string): Promise<WorkItem | null> {
    try {
        const connection = await getConnection(username);
        const query = `
            SELECT
...
/**
 * Fetches multiple work items by their names.
 */
export async function fetchWorkItemsByNames(username: string, workItemNames: string[]): Promise<WorkItem[]> {
    try {
